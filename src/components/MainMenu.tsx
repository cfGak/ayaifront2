...
const MainMenu: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        //const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/contributions`);
        const response = await axios.get<Contribution[]>(`${process.env.NEXT_PUBLIC_API_URL}/contributions`);
        ...
        {contributions.length > 0 ? (
          <div className="container mx-auto grid grid-cols-1 md:grid-cols-2 gap-8">
            {contributions.map((contribution, index) => (
              <div key={index} className="bg-white text-black rounded-lg shadow-md p-6 flex flex-col items-center justify-center">
                <h2 className="text-xl font-bold">ID: {contribution.employeeId}</h2>
                <p>Fecha: {new Date(contribution.date).toLocaleDateString()}</p>
                <p>AFP: {contribution.amount_afp}</p>
                <p>ISAPRE: {contribution.amount_isapre}</p>
                <p>Seguro: {contribution.amount_seguro}</p>
                <p>Total: {contribution.amount_total}</p>
              </div>
            ))}
          </div>
        ) : (
